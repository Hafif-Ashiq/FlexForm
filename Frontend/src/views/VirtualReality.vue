<template>
  ...
  <div class="outer-div" v-if="loaded && published">
    <div class="screen-form">
      ...
      <form class="sections-div" @submit.prevent="handleSubmit">
            ...
            <div v-for="(question, index) in section['questions']" class="quest"  :key="index">
              ...
              <div class="inputField" >
                ...
                <input v-model="fields[question['index']]['answer']" v-if="question['type'] == 'email'" class="input"
                  type="email" :placeholder="question['data']['placeholder']" :required="question['isRequired']">
                ...
                <div v-if="question['type'] === 'dropdown'">
                  <select :name="question['index']" class="input" v-model="fields[question['index']]['answer']"
                    ...
                    <option v-for="(option, index) in question['data']['options']" :key="index" :value="option">{{ option
                    }}</option>
                  </select>
                </div>
        ...
        <soft-button class="my-4 mb-2 py-4 button" size="lg" color="info">Submit Form</soft-button>
  ...
  <div class="unavailable-form" v-else-if="!published">
    <div class="unavailable-data"> 
      <svg fill="#ff9500" width="165px" height="165px" viewBox="0 0 32.00 32.00" version="1.1" xmlns="http://www.w3.org/2000/svg" stroke="#ff9500" stroke-width="0.00032"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#CCCCCC" stroke-width="0.384"></g><g id="SVGRepo_iconCarrier"> <title>exclamation-round</title> <path d="M0 16q0 3.264 1.28 6.208t3.392 5.12 5.12 3.424 6.208 1.248 6.208-1.248 5.12-3.424 3.392-5.12 1.28-6.208-1.28-6.208-3.392-5.12-5.088-3.392-6.24-1.28q-3.264 0-6.208 1.28t-5.12 3.392-3.392 5.12-1.28 6.208zM4 16q0-3.264 1.6-6.016t4.384-4.352 6.016-1.632 6.016 1.632 4.384 4.352 1.6 6.016-1.6 6.048-4.384 4.352-6.016 1.6-6.016-1.6-4.384-4.352-1.6-6.048zM12 10.016q0 0.992 0.512 2.976t0.992 3.52l0.512 1.504q0 0.832 0.576 1.408t1.408 0.576 1.408-0.576 0.608-1.408l1.984-8q0-1.664-1.184-2.848t-2.816-1.152-2.816 1.152-1.184 2.848zM14.016 24q0 0.832 0.576 1.44t1.408 0.576 1.408-0.576 0.608-1.44-0.608-1.408-1.408-0.576-1.408 0.576-0.576 1.408z"></path> </g></svg>
    <p>Form Not Available</p>
    </div>
  </div>
  <div class="unavailable-form" v-if="success_submit">
    <div class="submitted-data"> 
      <svg viewBox="0 0 512 512" width="200px" height="200px" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#80ff00" stroke="#80ff00"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>success-filled</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="add-copy-2" fill="#00bd6b" transform="translate(42.666667, 42.666667)"> <path d="M213.333333,3.55271368e-14 C95.51296,3.55271368e-14 3.55271368e-14,95.51296 3.55271368e-14,213.333333 C3.55271368e-14,331.153707 95.51296,426.666667 213.333333,426.666667 C331.153707,426.666667 426.666667,331.153707 426.666667,213.333333 C426.666667,95.51296 331.153707,3.55271368e-14 213.333333,3.55271368e-14 Z M293.669333,137.114453 L323.835947,167.281067 L192,299.66912 L112.916693,220.585813 L143.083307,190.4192 L192,239.335893 L293.669333,137.114453 Z" id="Shape"> </path> </g> </g> </g></svg>
...
export default {
  ...
  methods: {
    handleSubmit() {
      ...
      this.fileFields.forEach(file => {
        this.fields[file['index']]['answer'] = file['file']
      })

      this.checkBoxArray.forEach(options => {

        this.fields[options['index']]['answer'] = JSON.stringify(options['array'])
      })
      // console.log(this.fields);

      const data = {}
      for (let i = 0; i < this.fields.length; i++) {
        data[this.fields[i]['field']] = this.fields[i]['answer']
        console.log(data[this.fields[i]['field']]);
      }
      
      console.log(data);
      axios.post("http://localhost:8000/post-response/" + this.id,data,
        {
          headers: {
    ...
    handleCheckBox(index, option) {
      ...
      if (presentIndex !== -1) {
        let optArray = this.checkBoxArray[presentIndex]['array']
  ...
  async mounted() {
    console.log(this.id);
    const getdata = await axios.get(this.url + "get-form/" + this.id)
...
.screen-form {
  ...
  border-radius: 30px;
  background-color: white;
  box-shadow: -2px 2px 10px 1px rgb(143, 143, 143);
}
